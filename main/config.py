""" Конфиг """
WIDTH = 1000
HEIGHT = 600
FPS = 30
APP_NAME = "Port Scanner"
VERSION = ' v0.8'
INFORMATION = (
    "Help:\n\n\n"
    "Check your IP - displays all IP addresses of computers on the network\n\n"
    "Check all ports - displays all open ports on each IP address of a computer on the network\n\n"
    "All info - displays information about: IP addresses, open ports, network driver and other data\n\n"
    "Custom task - a function through which you can find out open ports on a specified IP address\n\n"
    "Log - displays all program logs"
)
